...
%
\subsection{Classificações e taxonomias de vunerabilidades}
O primeiro passo para que o desenvolvedor consiga cuidar de vunerabilidades no código fonte é conhecer quais são os problemas mais comuns existentes em softwares e como os atancantes utilizam estas vunerabilidadespara falhar o sistema. Existem muitos tipos de vunerabilidades, e com o passar do tempo, novas vunerabilidades são descobertas e novos exploits são criados por atacantes. Em meio a essa diversidade, a classificação de vunerabilidades representa um grande desafio, porém, já houve vários avanços na área com o objetivo de enumerar e catalogar estas vunerabilidades.

%

O CVE (\emph{Common Vunerabilities and Exposures}) é um projeto criado pelo MITRE com o objetivo de enumerar as vunerabilidades descoberdas pela comunidade para facilitar o compartilhamento de informações. Como é mostrado em MARTIN, antigamente cada organização nomeava de sua maneira uma vunerabilidade, de forma que uma mesma vunerabilidade era referenciada de maneira completamente distinta entra as organizações. 
...
O PLOVER foi criado em 2005 pelo MITRE em parceria com o DHS(US. Departament of Homeland Secutity) e o NIST(National Institute of Technology). É um documento que lista mais de 1400 exemplos reais de vunerabilidades identificados pelo CVE. Trata-se de um framework conceitual que descreve as vunerabilidades em diversos níveis de detalhe. O PLOVER também fornece uma série de conceitos que podem ajudar na comunicação e discussões à respeitos de vunerabilidades.

%

Foram definidas 28 categorias de mais alto nivel para a categorização de vunerabilidades. Algumas dessas são:

\begin{itemize}
\item BUFF: inclui vunerabilidades de Buffer Overflow, formatação de strings, etc;
\item CRIPTO: inclui vunerabilidades relacionadas a criptografia;
\item SPECTS: inclui vunerabilidades que ocorrem e tecnologias específicas, como a injeção de SQL e XSS
\end{itemize}
...
O CLASP não é apenas uma taxonomia de categorização de vunerabilidades, mas sim um processo que busca melhorar a segurança de softwares. No de diz respeito a classificação, o CLASP define 5 categorias alto nível que incluem 104 tipos de vunerabilidades, que são:
...
%
Com este cenário apresentado acima, em que temos a CVE como projeto de enumerar as vunerabilidades encontradas e as taxonomias elaboradas por diversos trabralhos com o objetivo de classificar e dar mais informações sobre a vunerabilidade, ainda haviam a necessidade das empresas e organizações de utilizarem o uma terminologia padrão para listar e classificar vunerabilidades de software, gerando uma linguagem unificada e uma base para ferramentas e serviços de medição dessas vunerabilidades. Para essa necessidade foi elaborado o projeto CWE.

%
O CWE é uma lista formal de vunerabilidades comuns de software (\emph{Common Weakness Enumeration}). Tem como objetivo estabelecer uma linguagem comum para descrever vunerabilidades de software no design, arquitetura ou no código; servir de base para ferramentas de análise de cobertura de segurança de código, dessa forma é possivel saber quais vunerabilidades as ferramentas conseguem capturar; e prover uma base de informações padrão a respeito de como identificar, mitigar e previnir uma certa vunerabilidade. Dessa forma, diferente da enumeração fornecida pela CVE, O CWE também inclui detalhes sobre a vunerabilidade e cria uma classificação baseadas nos trabalhos desenvolvidos sobre taxonomias apresentados neste trabalho e outros que não foram apresentados.
...
Métricas de vunerabilidade são métricas diretas que contam quantas ocorrências da vunerabilidade é encontrada no software. Como visto na secção de Segurança(COLOCAR REFERNCIA PARA A SECÇAO), o projeto CWE busca definir  e classificar vunerabilidades descobertas pela comunidade levando em consideração os detalhes de como essa vunerabilidade ocorre no código para a geração de um erro. Considerando como referência o projeto CWE, vamos tomar como exemplo o erro de buffer overflow. Existem várias CWEs que especificam uma maneira diferente de se ter o erro de Buffer overflow. Encontrar a vunerabilidade no código consiste em identificar o uso do mesmo código ou lógica de programação que define a CWE. Métricas de segurança se baseiam então na ocorrencia ou não de trechos de código ou lógica de programação que representam uma vunerabilidade já catalogada.
...
O \emph{Common Vulnerability Scoring System - CVSS} é um \emph{framework} aberto desenvolvido pelo NIST (National Institute of Standard and Technology) em parceria com organizações industriais que endereça as questões relacionadas as métricas para quantificar a severidade e risco de vulnerabilidades \cite{cvss2007}. Com o CVSS é possível estabelecer um escore entre as vunerabilidades de maneira que possa haver uma comparação entre a criticidade de cada vunerabilidade para o contexto da organização. Distinguindo quais vunerabilidades são mais graves, os gestores conseguem distribuir melhor o esforço para o tratamento dessas vunerabilidades mais críticas. O CVSS é composto por três grupos de métricas: Básicas, Temporal e Ambiental.
