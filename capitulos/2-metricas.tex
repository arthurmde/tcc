...
O CVE (\emph{Common Vunerabilities and Exposures}) é um projeto criado pelo MITRE com o objetivo de enumerar as vulnerabilidades descoberdas pela comunidade para facilitar o compartilhamento de informações. Como é mostrado em MARTIN, antigamente cada organização nomeava de sua maneira uma vunerabilidade, de forma que uma mesma vunerabilidade era referenciada de maneira completamente distinta entra as organizações. 
...
%
Exemplificando, as vulnerabilidades do tipo Buffer Overflow se encaixariam na categoria de erro de tipo e de range, pois nesse tipo de vunerabilidade é permitido a escrita de uma informação além o limite do buffer. A vunerabilidade do tipo  injeção de SQL também se encaixaria nessa categoria, pois esse tipo de vunerabilidade ocorre quando não há validação no tipo de informação fornecida pelo usuário. Mais detalhes podem ser vistos em http://cwe.mitre.org/about/sources.html (ARRUMAR REFERENCIA).
...
%
Com este cenário apresentado acima, em que temos a CVE como projeto de enumerar as vulnerabilidades encontradas e as taxonomias elaboradas por diversos trabralhos com o objetivo de classificar e dar mais informações sobre a vunerabilidade, ainda haviam a necessidade das empresas e organizações de utilizarem o uma terminologia padrão para listar e classificar vulnerabilidades de software, gerando uma linguagem unificada e uma base para ferramentas e serviços de medição dessas vulnerabilidades. Para essa necessidade foi elaborado o projeto CWE.

%
O CWE é uma lista formal de vulnerabilidades comuns de software (\emph{Common Weakness Enumeration}). Tem como objetivo estabelecer uma linguagem comum para descrever vulnerabilidades de software no design, arquitetura ou no código; servir de base para ferramentas de análise de cobertura de segurança de código, dessa forma é possivel saber quais vulnerabilidades as ferramentas conseguem capturar; e prover uma base de informações padrão a respeito de como identificar, mitigar e previnir uma certa vunerabilidade. Dessa forma, diferente da enumeração fornecida pela CVE, O CWE também inclui detalhes sobre a vunerabilidade e cria uma classificação baseadas nos trabalhos desenvolvidos sobre taxonomias apresentados neste trabalho e outros que não foram apresentados.
...
Atualmente existem várias ferramentas de análise estática que detectam vulnerabilidades no código fonte. Essas ferramentas utilizam de divesas tecnicas de detecção e buscam encontrar tipos específicos de vulnerabilidades. Como visto na secção de Segurança(COLOCAR REFERNCIA PARA A SECÇAO), o projeto CWE busca definir  e classificar vulnerabilidades descobertas pela comunidade levando em consideração os detalhes de como essa vunerabilidade ocorre no código para a geração de um erro. Considerando como referência o projeto CWE, vamos tomar como exemplo o erro de buffer overflow. Existem várias CWEs que especificam uma maneira diferente de se ter o erro de Buffer overflow. Encontrar a vunerabilidade no código consiste em identificar o uso do mesmo código ou lógica de programação definidas em CWEs relacionadas a esse erro. Dessa forma, as ferramentas de análise estática buscam em econtrar vulnerabilidades específicas, considerando o padrão CWE ou não, e quantificam o numero de ocorrências dessas vulnerabilidades por projeto, arquivo, método ou linha de código.
...
\begin{itemize}
\item Número total de vulnerabilidades no projeto: Cosiste em quantificar o total de vulnerabilidades encontradas no projeto inteiro, somando todos os tipos de vulnerabilidades encontradas;
\item Número de vulnerabilidades por arquivo:
\item Número de vulnerabilidades por função:
\item Quantidade de um vunerabilidade especifica no projeto:
\item Quantidade de um vunerabilidade especifica por arquivo:
\item Quantidade de um vunerabilidade especifica por função:
\end{itemize}
...
%Continuando com o exemplo da vunerabilidade de Buffer Overflow, podemos citar duas CWEs que identificam essa vunerabilidade de maneiras diferentes: CWE 242 e CWE 120. A CWE 120 é denominada \emph{Classic Buffer Overflow}, e esta vunerabilidade ocorre quando quando um buffer de entrada é copiado para um buffer de saída sem que haja a verificação o tamanho da entrada é menor ou igual ao tamanho de saída, podendo gerar uma sobrescrita nas posições de memória além das posições utilizadas pelo buffer. Da mesma maneira, a CWE 242 também trata o risco de um buffer overflow com o uso da função "gets()" da linguagem C, que  copia os dados de input do usuário para um buffer, sendo que não há garantia de que o input será menor ou igual o limite do buffer destinado ao armazenamento da informação.

%

%A identificação desse códigos que representam vulnerabilidades específicas podem ser feitas com o auxílio de ferramentas de análise estática de código, justamente pelo fato de que é difícil para um programador saber se ele está inserindo uma vunerabilidade no código ou não, pois é preciso ter o conhecimento da vunerabilidade. A ferramenta de análise estática Analizo, por exemplo, fornece algumas métricas de vunerabilidade. Estas são reportadas indicando a quantidade e o local de ocorrência.
%
...
%
As métricas do grupo Temporal podem ser desconsideradas do cálculo, sendo opcionais. Este grupo de métricas representam características de uma vunerabilidade que podem mudar ao longo do tempo mas não mudam entre ambientes de usuário.



%

As métricas do grupo Ambiental também são opcionais ao calculo, podendo ser desconsideradas. Este grupo de métricas representam características de uma vunerabilidade relevantes a um particular ambiente de usuário. Ou seja, são características que estão relacionadas unicamente ao ambiente em que a vunerabilidade é analisada.
%  
