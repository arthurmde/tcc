...
\item \textbf{NOC (\emph{Number of Children} - Número de Filhos)}: NOC consiste no número de filhos direto de uma classe. (FALAR MAIS AQUI)
...
\item \textbf{COF (\emph{Coupling Factor} - Fator de Acoplamento)}: EXPLICAR ESTA AQUIIIIIIIIII
\item \textbf{LCOM (\emph{Lack of Cohesion in Methods} - Ausência de Coesões em Métodos)}: EXPLICAR ESTA AQUIIIIIIIIII
\end{itemize}
