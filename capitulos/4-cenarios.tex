...
Nesta seção propomos quatro Cenários de Decisões. Um estudo muito interessante realizado por Chowdhury e colaboradores (\citeyear{chowdhury}), apresenta três novas formas de medir aspectos de segurança de um software a partir de suas estruturas estáticas, que poderiam ser representadas e evoluídas para Cenários de Decisões:

\begin{itemize}
\item \textbf{\emph{Stall Ratio}}: representa o quanto o progresso de um programa é impedido por atividades sem importância dentro do código. Os autores afirma que linhas de código que não contribuem para o objetivo principal do programa provocam atrasos consideráveis para que o mesmo assuma um estado estável e aceitável. Além disso, os autores enfatiza que esse tipo de problema é mais crítico quando encontrado dentro de laços de repetição.
\item \textbf{\emph{Coupling Corruption Propagation}}: caracteriza o impacto no total de métodos que poderiam ser afetados por erros existentes em métodos originais, herdados por exemplo.
\item \textbf{\emph{Critical Element Ratio}}: representa a proporção de elementos críticos de um objeto em relação ao total de elementos daquele mesmo objeto. Neste sentido, os autores caracterizam elementos críticos como elementos que serão manipulados ao longo do programa e não só no contexto do objeto criado.
\end{itemize}
...
\item \textbf{Nível}: Método;
\item \textbf{Descrição}: Este cenário busca identificar no código fonte ocorrência de operações lógicas que podem estar comprometidas por utilizarem variáveis indefinidas ou não inicializadas, compromentendo então a integridade da operação.
